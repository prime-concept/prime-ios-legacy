module CodeCoverage
    ...
    CODE_COVERAGE_BUILD_PARENT_PATH ||= "."
    CODE_COVERAGE_INTERMEDIATES_DIR ||= '.'
    ...
    def self.flush
        ...
        if self.on?
            ...
            CI::Application.flush_code_coverage
        end
    end
    ...
    def self.generate_lcov_baseline_info_file(path_to_info_files=@code_coverage_folder_path)
        ...
        if self.on?
            ...
            `lcov --capture --initial --base-directory . --directory #{@project_intermediates} --output-file #{baseline_info_file_path}`
        end
    end
    ...
    def self.generate_lcov_info_file(scenario)
      if self.on?
        ...
        `lcov --capture --base-directory . --directory #{@project_intermediates} --output-file #{info_file_path}`
      end
    end
    ...
    def self.generate_lcov_reports_from_info_file(info_file=nil, dest_dir="#{@code_coverage_folder_path}")
        ...
        puts("[CODE COVERAGE] Generating code coverage report: #{report_folder_path}/index.html")
        `genhtml --ignore-errors source #{info_file} --legend --title "#{@app_name} Code Coverage Report" --output-directory=#{report_folder_path}`
      end
    end
    ...
    def self.open_report(dest_dir="#{@code_coverage_folder_path}")
        report_folder_path = File.join(dest_dir, 'reports', 'index.html')
        `open #{report_folder_path}`
    end
end