...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position
{
    ...
    if (!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
        case SVPullToRefreshPosition_Top:
            yOrigin = -SVPullToRefreshViewHeight;
            break;
        case SVPullToRefreshPosition_Bottom:
...
{
    [self addPullToRefreshWithActionHandler:actionHandler position:SVPullToRefreshPosition_Top];
}

- (void)triggerPullToRefresh
{
    self.pullToRefreshView.state = SVPullToRefreshState_Triggered;
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.state = SVPullToRefreshState_Stopped;
...
- (void)layoutSubviews
{
    ...
    if (hasCustomView) {
        ...
        switch (self.state) {
        case SVPullToRefreshState_All:
        case SVPullToRefreshState_Stopped:
            self.arrow.alpha = 1;
        ...
        case SVPullToRefreshState_Triggered:
            switch (self.position) {
            case SVPullToRefreshPosition_Top:
                [self rotateArrow:(float)M_PI hide:NO];
                break;
        ...
        case SVPullToRefreshState_Loading:
...
{
    if (self.state != SVPullToRefreshState_Loading) {
        ...
        if (!self.scrollView.isDragging && self.state == SVPullToRefreshState_Triggered)
            self.state = SVPullToRefreshState_Loading;
        else if (contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == SVPullToRefreshState_Stopped && self.position == SVPullToRefreshPosition_Top)
            self.state = SVPullToRefreshState_Triggered;
        else if (contentOffset.y >= scrollOffsetThreshold && self.state != SVPullToRefreshState_Stopped && self.position == SVPullToRefreshPosition_Top)
            self.state = SVPullToRefreshState_Stopped;
        else if (contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == SVPullToRefreshState_Stopped && self.position == SVPullToRefreshPosition_Bottom)
            self.state = SVPullToRefreshState_Triggered;
        else if (contentOffset.y <= scrollOffsetThreshold && self.state != SVPullToRefreshState_Stopped && self.position == SVPullToRefreshPosition_Bottom)
            self.state = SVPullToRefreshState_Stopped;
    } else {
        CGFloat offset;
...
- (void)setTitle:(NSString*)title forState:(SVPullToRefreshState)state
{
    ...
    if (state == SVPullToRefreshState_All)
...
- (void)setState:(SVPullToRefreshState)newState
{
    ...
    case SVPullToRefreshState_Loading:
        ...
        if (previousState == SVPullToRefreshState_Triggered && pullToRefreshActionHandler)
